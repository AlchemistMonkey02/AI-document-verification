...
function loadRules(documentType) {
    ...
    switch (documentType) {
        ...
        case "DOC_ATTENDANCE": filename = "attendance.rules.json"; break;
        case "DOC_WORK_ORDER": filename = "work_order.rules.json"; break;
        case "DOC_COMPLETION_CERT": filename = "completion_certificate.rules.json"; break;
        // Legacy fallback
...
function runKeywordGate(docText, rules) {
    let keywords = rules.identification_keywords;
    if (!keywords && rules.identification && rules.identification.primary_keywords) {
        keywords = rules.identification.primary_keywords;
    }
    
    if (!keywords || keywords.length === 0) return { passed: true, hits: [], missing: [] };
    
    const lowerDocText = docText.toLowerCase();
    const hits = keywords.filter(kw => lowerDocText.includes(kw.toLowerCase()));
    
    // Strict Validation: Ensure that we meet min_keyword_hits if specified, otherwise at least 1
    const minHits = (rules.identification && rules.identification.min_keyword_hits) ? rules.identification.min_keyword_hits : 1;
    
    return {
        passed: hits.length >= minHits,
        hits: hits,
        missing: keywords.filter(kw => !lowerDocText.includes(kw.toLowerCase()))
    };
}
